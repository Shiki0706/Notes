...
\section{不确定性}
\subsection{偏好}
之前我们假设消费者在他的消费集$X$中的所有消费束$\x$上拥有偏好关系. 在加入不确定性因素之后, 我们只要稍微变动一下视角即可. 我们将保留偏好关系的符号, 但我们不再假设消费者对消费束具有偏好关系, 而是对\textbf{赌博} (gamble)具有偏好关系. 更正式地说, 令$A=\{a_1,\cdots,a_n\}$表示结果 (outcome)的有限集, 这些$a_i$可以为消费束、金额数量或其它任何东西, 关键在于$a_i$本身涉及不确定性, 并且我们使用$A$作为赌博的基.

例如令$A=\{1,-1\}$, 其中$1$代表赢得1元钱的结果, $-1$代表输掉1元钱的结果, 赌局就是投掷一枚质地均匀的硬币, 如果正面朝上则赢1元, 反之则输1元, 这两种结果发生的概率均为$1/2$. 

更一般地, \textbf{简单赌博} (simple gamble)对$A$中每个结果$a_i$赋予一个概率$p_i$, 显然每个$p_i$是非负的, 由于赌博必定导致$A$中某个结果发生, 因此$p_i$之和为1, 我们将简单赌博记为$(p_1\circ a_1,\cdots,p_n\circ a_n)$. 如果某个赌博的奖品也是赌博, 那么称这个赌博为\textbf{复合赌博} (compound gamble).

\begin{definition}[简单赌博]\label{def:def2.1}
令$A=\{a_1,\cdots,a_n\}$表示结果集, 那么$A$上的简单赌博集为
$$\mathcal{G}_S\equiv\left\{(p_1\circ a_1,\cdots,p_n\circ a_n)\left|p_i\ge0, \sum_{i=1}^{n}p_i=1\right.\right\}$$
\end{definition}
特别地, 如果某些$p_i=0$, 那么就将相应的项从表达式中去掉. 如果对于每个$i$都有$p_i=1$, 那么将$1\circ a_i$简写为$a_i$, 表示必然发生结果$a_i$的赌博, 此时$\mathcal{G}_S$就包含了选择集$A$. 回到投掷硬币的例子中, 每个人都面对着简单赌博$(\frac{1}{2}\circ 1, \frac{1}{2}\circ -1)$.

最后设$\mathcal{G}=\bigcup_{j=0}^\infty \mathcal{G}_j$表示全体赌博构成的集合, 其中$\mathcal{G}_0=A$, 并且$\mathcal{G}_j=\{(p_1\circ g^1,\cdots, p_k\circ g^k)|k\ge1; p_i\ge0, g^i\in \mathcal{G}_{j-1}, \forall i=1,\cdots,k; \sum_{i=1}^{k}p_i=1\}$.

不确定情形下的选择的就是赌博. 与消费者理论对应, 假设决策制定者在赌博集$\mathcal{G}$上有偏好$\succsim$, 接下来还需要对决策制定者的偏好关系$\succsim$设置一组公理, 称为\textbf{不确定性下的选择公理} (axiom of choice under uncertainty).

\begin{axiom}[完备性]\label{axi:axi2.1}
对于$\mathcal{G}$中的任意两个赌博$g$和$g'$, 要么$g\succsim g'$, 要么$g'\succsim g$.
\end{axiom}

\begin{axiom}[传递性]\label{axi:axi2.2}
对于$\mathcal{G}$中的任意三个赌博$g$, $g'$和$g''$, 如果$g\succsim g'$并且$g'\succsim g''$, 那么$g\succsim g''$.
\end{axiom}

由于可以将$A$中的每个$a_i$视作$\mathcal{G}$中的退化赌博, 公理\ref{axi:axi2.1}和\ref{axi:axi2.2}意味着$A$的有限个元素可以被$
\succsim$排序. 不失一般性, 假设我们已经对$A$的元素进行了标识, 从而$a_1\succsim a_2\succsim\cdots\succsim a_n$. 因此在所有赌博中, 最好的是以概率1产生结果$a_1$的赌博, 而最差的是以概率1产生结果$a_n$的赌博.

\begin{axiom}[连续性]\label{axi:axi2.3}
对于$\mathcal{G}$中的任意赌博$g$, 存在某个概率$\alpha\in[0,1]$使得$g\sim (\alpha\circ a_1,(1-\alpha)\circ a_n)$.
\end{axiom}
举例来说, 假设$A=\{1000\text{元},100\text{元},\text{死亡}\}$, 对于绝大多数人都有$1000\text{元}\succ100\text{元}\succ\text{死亡}$, 但得到1000元需要开车出门去拿, 100元是送上门来, 而开车伴随着很小的死亡的正概率, 这样人就会在1000元和100元中进行选择. 倘若可以增加死亡概率直到这两个选择对某人来说是无差异的, 那么就是找到一个让她觉得无差异的概率.

\begin{axiom}[单调性]\label{axi:axi2.4}
对于所有概率$\alpha,\beta\in[0,1]$, $(\alpha\circ a_1,(1-\alpha)\circ a_n)\succsim (\beta\circ a_1,(1-\beta)\circ a_n)$当且仅当$\alpha\ge\beta$.
\end{axiom}

正如单调性要求的, 大多数人通常偏好以更高概率产生更好结果的赌博, 但事实未必总是如此. 例如, 对于长途跋涉的狩猎者来说, 死亡可能是这种行动的最差结果, 然而死亡的可能性增加了狩猎活动的刺激性. 因此, 狩猎者更喜欢较小死亡概率而不是绝对安全,这显然违背了单调性.
\chapter{生产者理论}
