%!TEX program = xelatex
\documentclass[cn,cyan,14pt,screen,bibstyle=gb7714-2015]{elegantnote}
\title{双重差分设计}
\date{\today}
...
\newpage
\section{交错时间处置}
\subsection{引子: 异质性处置效应下的TWFE估计量}
...
在以上假设成立的情况下, 仍然考虑使用静态TWFE模型
$$Y_{i,t}=\alpha_i+\lambda_t+D_{i,t}\beta_\text{post}+\epsilon_{i,t}$$
Goodman-Bacon (2021)证明了以下概率极限
$$\underset{N\to\infty}{\text{plim}}\hat{\beta}_\text{post}=\text{VWATT}+\text{VWCT}+\Delta \text{ATT}$$
其中VWATT是TWFE-DiD估计量所能估计的我们感兴趣的因果效应, VWCT在平行趋势假设成立的情况下为0, $\Delta \text{ATT}$等于各组群在处置前后的处置结果变化的加权和. 

特别地, 如果处置效应是同质的 (也即ATT跨组群和时点保持不变), 那么$\Delta\text{ATT}=0$, 此时$\beta_\text{post}$正确识别了因果关系, 使用OLS估计静态TWFE模型是合理的.

根据上一节的论述, 如果在不同时点受到的处置效应存在异质性, 那么TWFE估计是有偏的 (de Chaisemartin and D'Haultfœuille, 2020), 甚至会出现完全相反的因果效应. 事实上, 根据FWL定理可知$\hat{\beta}_{\text{post}}$的OLS估计量为
...
Goodman-Bacon (2021)进一步指出, TWFE-DiD估计量是所有可能的$2\times 2\,$DiD估计量的加权平均, 各个小的DiD估计量的处置组和控制组是混乱的, 有的将从未接受处置的组群作为控制组, 有的将尚未处置 (not-yet-treated)的组群作为控制组, 还有的将先接受处置的组群作为控制组而将后接受处置的组作为处置组. 当存在随时间变化的处置效应异质性时, 某些小的DiD估计量就会产生负权重. 特别地, 使用较早接受处置的组作为控制组被称为\textbf{禁止比较} (forbidden comparison). 
