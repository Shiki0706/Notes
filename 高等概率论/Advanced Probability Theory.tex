...
\begin{definition}
设$\{\F_n:n\ge0\}$是一个滤链, 称$\{H_n:n\ge1\}$是可预测序列 (predictable sequence), 如果对一切$n\ge1$都有$H_n\in\F_{n-1}$. 
\end{definition}
...
\begin{theorem}\label{thm:thm4.6}
  设$\{X_n:n\ge0\}$是一个上鞅, 如果$\{H_n\ge0\}$是可预测的且每个$H_n$有界, 那么$\{(H\cdot X)_n\}$是一个上鞅.
\end{theorem}
\begin{proof}
...
\end{remark}
\begin{definition}
随机变量$N$称为停时 (stopping time), 如果对于一切$n<\infty$都有$\{N=n\}\in\F_n$. 换言之, 事件$\{N=n\}$发生与否完全由$\F_n$决定.
\end{definition}

根据$\{N=n\}=\{N\leq n\}\backslash \{N\leq n-1\}$可知, 停时的等价定义为对于一切$n<\infty$都有$\{N\leq n\}\in\F_n$; 如果$N$为停时, $k$为正整数, 那么$N+k$仍为停时, 而$N-k$不一定为停时, 这是因为事件$\{N=n+k\}$的发生不一定取决于$\F_n$.

假设$H_n=1_{\{N\ge n\}}$, $N$为停时, 那么$\{N\ge n\}=\{N\leq n-1\}^c\in\F_{n-1}$, 因此$\{H_n\}$是可预测的, 根据定理\ref{thm:thm4.6}可知, 如果$\{X_n\}$是一个上鞅, 那么$(H\cdot X)_n=X_{N\wedge n}-X_0$定义了一个上鞅, 容易得到以下定理.
\begin{theorem}
  如果$N$是一个停时, $\{X_n\}$是一个(上或下)鞅, 那么$\{X_{N\wedge n}\}$也是. 
\end{theorem}

\begin{example}
设随机变量$H_n$为$n$时刻的买入量, 其完全取决于$n$时刻前的股价表现 ($H_{n}\in \F_{n-1}$), 故而$\{H_n\}$是可预测序列. 再设$X_n$为$n$时刻的股价, 且$\{X_n\}$是关于$\{\F_n\}$的鞅, 此时的累计收益可以表示为
...
$$H_m=\begin{cases}
        ...
        0, & \mbox{otherwise}
      \end{cases}$$
定义了一个可预测序列. 由于$X_{\tau_k}\leq a$且$X_{\sigma_k}\ge b$, 故而在时刻$\tau_k$和$\sigma_k$之间, $X_m$从$a$上穿$b$, 于是$H_m$是一个试图从上穿中获利的赌博系统. 在上面的例子中, 投资者通常是在$X_m\leq a$时买入股票, 然后在$X_m\ge b$时卖出.


\begin{theorem}[上穿不等式]
  如果$\{X_m:m\ge0\}$是一个下鞅, 那么
  $$(b-a)E(U_n)\leq E(X_n-a)^+-E(X_0-a)^+$$
  其中$U_n=\sup\{k: \sigma_k\leq n\}$为$n$时刻完成的上穿次数.
\end{theorem}
\begin{proof}
